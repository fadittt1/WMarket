...
import { Product, Pack } from "@/store/useStore";
...
type ProductItem = Product & { itemType: 'product' };
type PackItem = Pack & { itemType: 'pack' };
type GridItem = ProductItem | PackItem;
...
const ProductGrid = () => {
  ...
  const shuffledProducts = useMemo(() => {
    const arr = [...db.products].filter((p) => !p.packOnly);
  ...
  const allItems = useMemo<GridItem[]>(() => {
    ...
    const byCategory = filter === "All"
      ? shuffledProducts
      : shuffledProducts.filter((p) => p.categories?.includes(filter) || p.category === filter);

    const productItems: ProductItem[] = byCategory.map((p) => ({ ...p, itemType: 'product' as const }));

    const combined: GridItem[] = [...packItems, ...productItems];
    ...
    return combined;
  }, [db.packs, shuffledProducts, filter]);

  const filtered = search.trim()
    ? allItems.filter((item) => item.name.toLowerCase().includes(search.trim().toLowerCase()))
    : allItems;

  const totalReactions = (p: Product) => {
    const r = p.reactions || { love: 0, haha: 0, like: 0 };
  ...
  return (
    <div>
                ...
                return (
                  ...
                  >
                    ...
                    <div style={{ padding: '9px 11px 11px' }}>
                      ...
                      <div style={{ fontSize: '9px', color: '#8a7a65', textTransform: 'uppercase', letterSpacing: '.08em' }}>{pack.productIds.length} products included</div>
              ...
              return (
                ...
                >
                  ...
                  <div className="p-3 pb-3.5">
                    ...
                    <div className="text-[10px] text-muted-foreground uppercase tracking-wider mt-0.5">{p.categories?.length ? p.categories.join(", ") : p.category}</div>

                    <div className="flex items-center gap-2 mt-1.5 text-[10px] text-muted-foreground">
                      {totalReactions(p) > 0 && (
                        <span className="flex items-center gap-0.5">
                          <Heart className="w-3 h-3" /> {totalReactions(p)}
                        </span>
                      )}
      ...
      {selectedProduct && (
        <ProductModal
          product={db.products.find((p) => p.id === selectedProduct.id) || selectedProduct}
