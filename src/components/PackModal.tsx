...
const PackModal = ({ pack, onClose }: Props) => {
  ...
  const includedProducts = db.products.filter((p) => pack.productIds.includes(p.id));
            ...
            <div className="flex items-center gap-3">
              <div>
                ...
                <div className="text-[10px] text-muted-foreground uppercase tracking-wider">
                  {pack.productIds.length} product{pack.productIds.length !== 1 ? "s" : ""} included
                </div>
              </div>
            </div>
            ...
            <div className="px-4 py-3 border-b border-border/60">
              ...
              <div className="grid grid-cols-2 gap-2.5">
                {includedProducts.map((p) => (
                  <div key={p.id} className="rounded-xl p-2 flex flex-col items-center" style={{ backgroundColor: '#efe8d6' }}>
                    {p.img ? (
                      ...
                      <div className="w-full h-20 rounded-lg flex items-center justify-center" style={{ backgroundColor: '#e8dfc8' }}>
                        <span className="text-[52px] leading-none">{p.emoji || "📦"}</span>
                      </div>
                    )}
                    <span className="text-sm font-medium mt-1.5 text-center leading-tight font-sans">{p.name}</span>
                    <span className="text-xs font-display text-accent mt-0.5">{p.price.toFixed(3)} TND</span>
                  </div>
                ))}
              </div>
            </div>
