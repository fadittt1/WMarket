...
import { Product, ReactionType } from "@/store/useStore";
...
const ProductModal = ({ product, onClose }: Props) => {
  ...
  const handleComment = () => {
    ...
    addComment(product.id, currentUser.name, commentText.trim());
    setCommentText("");
  };
  ...
  const submitEdit = (commentId: string) => {
    if (editText.trim()) {
      editComment(product.id, commentId, editText.trim());
    }
    setEditingId(null);
    setEditText("");
  };
          ...
          <div className="flex items-center justify-between p-4 border-b border-border/60">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <div className="text-[10px] text-muted-foreground uppercase tracking-wider">{product.category}</div>
              </div>
            </div>
